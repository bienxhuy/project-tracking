...
export const projectSchema = z.object({
  ...
  year: z.number().int().min(2000, { message: "Năm học phải từ 2000 trở đi." }),
  semester: z.number().int().min(1).max(3, { message: "Học kỳ phải từ 1 đến 3." }),
  batch: z.string().trim().min(1, { message: "Đợt là bắt buộc." }),
  falculty: z.string().trim().min(2, { message: "Khoa phải có ít nhất 2 ký tự." }),
  startDate: z.string().trim().min(1, { message: "Ngày bắt đầu là bắt buộc." }),
  endDate: z.string().trim().min(1, { message: "Ngày kết thúc là bắt buộc." }),
