...
- **Response:** `ApiResponse<User[]>`
- **Note:** Frontend can parse response to `BaseUser[]` when only minimal user info is needed
```typescript
// Parse to BaseUser array
const baseUsers: BaseUser[] = response.data.map(user => ({
  id: user.id,
...
- **Method:** `PATCH`
- **URL:** `/api/v1/projects/{id}/content/lock`
- **Description:** Lock project objective and description (Instructor only)
- **Response:** `ApiResponse<null>`

### 3.7 Unlock Project Content
- **Method:** `PATCH`
- **URL:** `/api/v1/projects/{id}/content/unlock`
- **Description:** Unlock project objective and description (Instructor only)
- **Response:** `ApiResponse<null>`

### 3.8 Lock Entire Project
- **Method:** `PATCH`
- **URL:** `/api/v1/projects/{id}/lock`
- **Description:** Lock entire project including content, milestones, tasks, and reports (Instructor only)
- **Response:** `ApiResponse<null>`
- **Note:** This locks everything in the project, preventing students from making any changes

### 3.9 Unlock Entire Project
- **Method:** `PATCH`
- **URL:** `/api/v1/projects/{id}/unlock`
- **Description:** Unlock entire project including content, milestones, tasks, and reports (Instructor only)
- **Response:** `ApiResponse<null>`
- **Note:** This unlocks everything in the project, allowing students to make changes

### 3.10 Export Project as PDF
- **Method:** `GET`
- **URL:** `/api/v1/projects/{id}/export-pdf`
- **Description:** Export project details, milestones, tasks, and reports as a PDF file (Instructor only)
- **Response:** Binary file (application/pdf)
- **Response Headers:**
  - `Content-Type: application/pdf`
  - `Content-Disposition: form-data; name="attachment"; filename="project_{id}_{title}.pdf"`
  - `Content-Length: {size}`
- **Note:** This endpoint returns a file download. Frontend should handle the blob response and trigger a download.

### 3.11 Get Student's Projects
- **Method:** `GET`
- **URL:** `/api/v1/projects/student/{studentId}`
- **Description:** Get all projects for the current student
- **Query Parameters:**
  - `year?: number` (optional - filter by year)
  - `semester?: number` (optional - filter by semester; value: 1-2)
  - `batch?: string` (optional - filter by batch; value: 1-2)
- **Response:** `ApiResponse<Project[]>`
- **Security Note:** Only returns projects where current user is a member

### 3.12 Get Instructor's Projects
- **Method:** `GET`
- **URL:** `/api/v1/projects/instructor/{instructorId}`
...
- **Note:** Don't implement filter on this endpoint yet

### 3.13 Get Project Members
- **Method:** `GET`
- **URL:** `/api/v1/project-members/project/{projectId}`
- **Description:** Get list of all members in a project
- **Response:** `ApiResponse<BaseUser[]>`
```typescript
// Response data is parsed to BaseUser array
{
  id: number,
  displayName: string,
  email: string,
...
```
- **Note:** Frontend will parse the API response data to extract BaseUser array from project members
...
- **Note:** Backend only returns specified fields. When `include=tasks`, response includes `tasks` array;
...
- **Method:** `GET`
- **URL:** `/api/v1/tasks/{id}`
- **Description:** Get detailed task information, optionally including reports
- **Query Parameters:**
  - `include?: string` - If set to `"reports"`, includes reports array in the task
- **Response:** `ApiResponse<Task>` or `ApiResponse<TaskDetail>` (when `include=reports`)
- **Note:** Backend only returns specified fields. When `include=reports`, response includes `reports` array (may be empty); otherwise `reports` is `null`
- **Primary Usage:** Use with include=reports
...
- **Method:** `GET`
- **URL:** `/api/v1/reports/task/{taskId}`
...
- **Description:** Submit report and change status to SUBMITTED (Instructor only)
- **Response:** `ApiResponse<Report>`
- **Permission:** Instructor only
- **Note:** This changes report status to SUBMITTED (unlocked state)

### 6.6 Lock Report
- **Method:** `PATCH`
- **URL:** `/api/v1/reports/{id}/lock`
- **Description:** Lock report and change status to LOCKED (Instructor only)
- **Response:** `ApiResponse<Report>`
- **Permission:** Instructor only
- **Note:** This changes report status to LOCKED
